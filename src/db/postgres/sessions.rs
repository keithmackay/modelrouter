...
#[async_trait]
impl SessionRepository for PostgresDb {
    async fn find_or_create(&self, session: NewSession) -> anyhow::Result<Session> {
        ...
        if let Some(ref ext_id) = session.external_id {
            let existing = sqlx::query_as::<_, Session>(
                "SELECT id, user_id, external_id, project, created_at, last_seen, metadata
                 FROM sessions WHERE user_id = $1 AND external_id = $2 LIMIT 1",
            )
        ...
        let row = sqlx::query_as::<_, Session>(
            r#"INSERT INTO sessions (user_id, external_id, project, created_at, last_seen, metadata)
               VALUES ($1, $2, $3, $4, $5, '{}')
               RETURNING id, user_id, external_id, project, created_at, last_seen, metadata"#,
        )
        .bind(session.user_id)
