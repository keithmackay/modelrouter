    ...
    let edit_form = if rule.window != "target" {
        format!(
            r##"<form hx-post="/admin/budgets/{id}/edit" hx-target="{target}" hx-swap="outerHTML" style="display:inline;">
                <input type="hidden" name="limit_usd" value="{lusd}">
                <button type="submit" class="btn btn-secondary" style="font-size:0.8rem;padding:0.3rem 0.6rem;">Edit</button>
            </form>"##,
            id = rule.id,
            target = card_target,
        ...
        // Target rules: simplified edit inline form
        format!(
            r##"<form hx-post="/admin/budgets/{id}/edit" hx-target="{target}" hx-swap="outerHTML" style="display:inline;">
